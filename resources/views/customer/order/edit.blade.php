                    ...
                    <div class="col-xs-12 col-sm-6 col-md-6">
                        <form method="POST" action="{{ route('customer.store') }}">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control url" name="copy" value="<?php echo number_format($order->slide) ?>" required="" readonly>
                            </div>
                            <div class="form-group">
                            ...
                            <?php if ($order->status == 0 && $order->payment_status != 1): ?>
                                <div class="form-group">
                                    <button class="btn btn-danger" style="float: right;"  onclick="event.preventDefault(); document.getElementById('cancel-order').submit();" >Yêu cầu hủy đơn hàng</button>
                                </div>
                            <?php endif ?>
                        </form>
                    </div>
...
<?php if ($order->status == 0 && $order->payment_status != 1): ?>
    <form id="cancel-order" action="{{ route('customer.orderRemove') }}" method="POST" style="display: none;"> @csrf <input type="hidden" name="orderid" value="<?php echo $order->id ?>"></form>
<?php endif ?>
@endsection()
